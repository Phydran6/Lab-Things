#!/usr/bin/env bash
#
# Nginx Proxy Manager via Docker – distro-unabhängiges Setup
# Annahme: Aufruf als normaler User MIT sudo-Rechten (root nicht nötig).
#
set -euo pipefail

# ----------------------------- Konfiguration --------------------------------
SERVICE_DIR="/opt/docker/npm"            # Config / persistente Daten
HTTP_PORT="80"                           # Proxy HTTP
HTTPS_PORT="443"                         # Proxy HTTPS
ADMIN_PORT="81"                          # Web-UI (Admin)
...
$SUDO mkdir -p "${SERVICE_DIR}/data" "${SERVICE_DIR}/letsencrypt"
...
services:
  npm:
    ...
    volumes:
      ...
      - ${SERVICE_DIR}/letsencrypt:/etc/letsencrypt${MNT_OPT}
...
log "NPM starten (docker compose up -d)..."
if [ "$TARGET_USER" != "root" ]; then
...
log "Fertig! NPM Admin-UI: http://${HOST_IP}:${ADMIN_PORT}"
echo "   Default-Login:  admin@example.com / changeme   <-- beim ersten Login ändern!"
echo "   Proxy-Ports:    ${HTTP_PORT} (HTTP) / ${HTTPS_PORT} (HTTPS)"
echo "   Daten:          ${SERVICE_DIR}/data  +  ${SERVICE_DIR}/letsencrypt"
echo
echo "Wildcard *.phytech.de: in 'SSL Certificates' -> Let's Encrypt + DNS-Challenge"
echo "Provider Cloudflare wählen und API-Token hinterlegen."
echo
echo "Hinweis: docker-Gruppe in dieser Shell erst nach Re-Login aktiv -> sonst: newgrp docker"
